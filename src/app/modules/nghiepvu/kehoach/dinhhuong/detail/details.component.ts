...
export class ApiDinhHuongDetailsComponent implements OnInit {
    ...
    onSubmit(status) {
        ...
        //     this._serviceApi.execServiceLogin("95FB5EC8-307D-46B2-9E6C-3BE3CE5D0C4D", [{"name":"MA_TRANG_THAI","value":status},{"name":"NAM","value":nam},{"name":"TEN_KE_HOACH","value":name},{"name":"TONG_HOP","value":0},{"name":"ORGID","value":"115"},{"name":"USERID","value":"STR"},{"name":"CAP_TAO","value":capTao}]).subscribe((data) => {
        ...
        //                                 if(listKeHoachChiTietDelete != null && listKeHoachChiTietDelete.length >0){
        //                                     listKeHoachChiTietDelete = listKeHoachChiTietDelete.filter(c => c.maKeHoachChiTiet !=itemChiTiet4.maKeHoachChiTiet); 
        //                                 }   
        ...
        //                             if(listKeHoachChiTietDelete != null && listKeHoachChiTietDelete.length >0){
        //                                 listKeHoachChiTietDelete = listKeHoachChiTietDelete.filter(c => c.maKeHoachChiTiet !=itemChiTiet.maKeHoachChiTiet); 
        //                             }    
        ...
        //                             this._serviceApi.execServiceLogin("8FC11318-019D-4132-989D-F47AF7386F53", [{"name":"DON_VI_CHU_TRI","value":itemChiTiet.DonViChuTri},{"name":"DU_TOAN","value":itemChiTiet.DuDoan},{"name":"MA_DON_VI","value":itemChiTiet.maDonVi},{"name":"NOI_DUNG","value":itemChiTiet.NoiDungHoatDong},{"name":"MA_KE_HOACH","value":itemKeHoach},{"name":"CHU_NHIEM_NHIEM_VU","value":itemChiTiet.ChuNhiemNhiemVu},{"name":"NOI_DUNG_DANG_KY","value":itemChiTiet.NoiDungDangKy},{"name":"CAP_TAO","value":"DONVI"},{"name":"MA_NGUON_KINH_PHI","value":itemChiTiet.NguonKinhPhi},{"name":"THOI_GIAN_THUC_HIEN","value":itemChiTiet.ThoiGianDuKien},{"name":"MA_NHOM","value":chitiet2.manhom},{"name":"ORGID","value":"STR"},{"name":"USERID","value":"STR"}]).subscribe((data) => {
        //                                 console.log("kehoachchitiet5:" + JSON.stringify(data));
        //                                 })
        //                         }
        //                     }
        ...
        //xóa chi tiet ke hoach
        // if(listKeHoachChiTietDelete != null && listKeHoachChiTietDelete.length >0){
        //     for(let i=0 ;i < listKeHoachChiTietDelete.length;i++){
        //         this._serviceApi.execServiceLogin("28157052-A3B7-45D4-85FD-6F2A10CE4CE1", [{"name":"MA_KE_HOACH_CTIET","value":listKeHoachChiTietDelete[i].maKeHoachChiTiet}]).subscribe((data) => {

        //         })
        //     }
        // }
        // //xóa file
        // let listUploadInsert =this.listupload.filter(c => c.mafile =='');
        // let listUploadMaFile =this.listupload.filter(c => c.mafile !='');
        // if(listUploadMaFile !=undefined && listUploadMaFile.length >0 && this.listFileDelete != undefined && this.listFileDelete.length >0){
        //     for(let i=0 ;i < listUploadMaFile.length;i++){
        //         this.listFileDelete = this.listFileDelete.filter(c => c.mafile != listUploadMaFile[i].mafile)
        //     }

        //     if(this.listFileDelete != null && this.listFileDelete.length >0){
        //         for(let i=0 ;i < this.listFileDelete.length;i++){
        //         this._serviceApi.execServiceLogin("83C28EE1-6A5A-4ADA-B866-287EBAC8B5D5", [{"name":"MA_FILE","value":this.listFileDelete[i].mafile}]).subscribe((data) => {
        ...
        // if(listUploadInsert != undefined && listUploadInsert.length >0){

        //     var token = localStorage.getItem("accessToken");
        //     for(let i=0 ;i < listUploadInsert.length;i++){

        //         let fileBase64 =listUploadInsert[i].base64.split(',')[1];


        //         this._serviceApi.execServiceLogin("2120F73B-854B-408E-B5AD-257E7F184922", [{"name":"FILE_NAME","value":listUploadInsert[i].fileName},{"name":"FILE_UPLOAD","value":fileBase64},{"name":"TOKEN_LINK","value":"Bearer "+token}]).subscribe((data) => {
        //             let result1 = "";
        //             if(data.data != null && data.data.createdAt != null ){
        //                 result1 = new Date(data.data.createdAt).toLocaleDateString('en-GB');
        //          }     
