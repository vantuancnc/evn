...
export class ApiDinhHuongDetailsComponent implements OnInit {
    ...
    updateDate() {
        if (this.idParam != undefined && this.idParam != null) {
            this._serviceApi
                .execServiceLogin('DC2F3F51-09CC-4237-9284-13EBB85C83C1', [
    ...
    downloadExcel() {
        this._serviceApi
            .execServiceLogin('B186CEDA-876B-4511-96D1-E199926A6913', [
                { name: 'ORGID', value: '115' },
            ])
            .subscribe((data) => {
                this.downloadTempExcel(
                    data.data,
                    'MAU_DANG_KY_DINH_HUONG.xlsx'
                );
            });
    }
    ...
    importFile(event) {
        ...
        reader.onload = () => {
            ...
            this._serviceApi
                .execServiceLogin('1E707636-93B5-43EA-97BC-2F850C14D1E3', [
                    { name: 'ORGID', value: '115' },
                    { name: 'FILE_UPLOAD', value: fileBase64 },
                ])
                .subscribe((data) => {
                    let arr = data.data || [];
                    let capTao = 'DONVI';
                    ...
                    let kehoach = { capTao: capTao, listKeHoach: arr };
    ...
    onSubmit(status) {
        ...
        for (let i = 0; i < this.form.value.listNhiemVu.length; i++) {
            for (
                let j = 0;
                j < this.form.value.listNhiemVu[i].listNhiemVu_cap2.length;
                j++
            ) {
                let chitiet2 =
                    this.form.value.listNhiemVu[i].listNhiemVu_cap2[j];
                if (this.listDonvi != undefined && this.listDonvi.length > 0) {
                    for (let k = 0; k < chitiet2.listNhiemVu_cap3.length; k++) {
                        let itemChiTiet = chitiet2.listNhiemVu_cap3[k];

                        if (
                            itemChiTiet.listNhiemVu_cap4 != undefined &&
                            itemChiTiet.listNhiemVu_cap4.length > 0
                        ) {
                            for (
                                let i = 0;
                                i < itemChiTiet.listNhiemVu_cap4.length;
                                i++
                            ) {
                                listChiTiet.push(
                                    itemChiTiet.listNhiemVu_cap4[i]
                                );
                            }
                        }
                    }
                    ...
                    if (
                        chitiet2.listNhiemVu_cap3 != undefined &&
                        chitiet2.listNhiemVu_cap3.length > 0
                    ) {
                        for (
                            let i = 0;
                            i < chitiet2.listNhiemVu_cap3.length;
                            i++
                        ) {
        ...
        this._serviceApi
            .execServiceLogin('404ABE65-3B92-448F-A8F0-9543503AE1E3', [
                { name: 'LIST_FILE', value: JSON.stringify(listFile) },
                {
                    name: 'LIST_KE_HOACH_CHI_TIET',
                    value: JSON.stringify(listChiTiet),
                },
                { name: 'TOKEN_LINK', value: 'Bearer ' + token },
                { name: 'KE_HOACH', value: JSON.stringify(kehoach) },
            ])
            .subscribe((data) => {
    ...
    deleteItemFile(items) {
        if (items.mafile != undefined && items.mafile != '') {
            this.listupload = this.listupload.filter(
                (item) => item.mafile != items.mafile
            );
        } else {
            this.listupload = this.listupload.filter(
